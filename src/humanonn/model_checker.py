...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="humanonn-check-models",
        description="Probe Humanonn model routes and report whether each model responds with the configured API keys.",
    )
    parser.add_argument(
        "--task",
...
def _probe_candidate(
    ...
    api_key_labels = settings.api_key_labels_for(candidate.provider)
    if not api_keys:
        return {
            "task": task,
    ...
    try:
        if candidate.provider == "groq":
            response = _call_groq(candidate, task, api_key_labels, max_output_tokens)
            return _ok_result(task, candidate, response)
        if candidate.provider == "gemini":
            response = _call_gemini(candidate, task, api_key_labels, image_path if task == "vision" else None, timeout_seconds)
            return _ok_result(task, candidate, response)
        if candidate.provider == "openrouter":
            response = _call_openrouter(candidate, task, api_key_labels, max_output_tokens, timeout_seconds)
            return _ok_result(task, candidate, response)
        if candidate.provider == "deepseek":
            response = _call_deepseek(candidate, task, api_key_labels, max_output_tokens, timeout_seconds)
            return _ok_result(task, candidate, response)
        if candidate.provider == "jina":
            response = _call_jina(candidate, task, api_key_labels, timeout_seconds)
            return _ok_result(task, candidate, response)
        return {
            "task": task,
            "bug_tag": candidate.bug_tag,
...
def _call_groq(candidate: ModelCandidate, task: str, api_key_labels: list[tuple[str, str]], max_output_tokens: int) -> dict[str, Any]:
    prompt = _text_prompt(task, candidate)
    last_error: Exception | None = None
    for api_key_label, api_key in api_key_labels:
        try:
...
def _call_gemini(candidate: ModelCandidate, task: str, api_key_labels: list[tuple[str, str]], image_path: Path | None, timeout_seconds: int) -> dict[str, Any]:
...
def _call_openrouter(candidate: ModelCandidate, task: str, api_key_labels: list[tuple[str, str]], max_output_tokens: int, timeout_seconds: int) -> dict[str, Any]:
    payload = {
        "model": candidate.model,
        "messages": [
            {"role": "system", "content": "Return JSON only."},
...
def _call_deepseek(candidate: ModelCandidate, task: str, api_key_labels: list[tuple[str, str]], max_output_tokens: int, timeout_seconds: int) -> dict[str, Any]:
    payload = {
        "model": candidate.model,
        "messages": [
            {"role": "system", "content": "Return JSON only."},
...
def _call_jina(candidate: ModelCandidate, task: str, api_key_labels: list[tuple[str, str]], timeout_seconds: int) -> dict[str, Any]:
    ...
    for api_key_label, api_key in api_key_labels:
        ...
        try:
            ...
            last_error = exc
    if last_error is None:
        raise RuntimeError("Missing Jina API key.")
    raise last_error
...
def _ok_result(task: str, candidate: ModelCandidate, response: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        ...
        "api_key_label": response.get("api_key_label", "unknown"),
    }
...
def _format_result(result: dict[str, Any]) -> str:
    api_key_label = result.get("api_key_label")
    api_suffix = f" | api={api_key_label}" if api_key_label and api_key_label != "unknown" else ""
    base = f"- {result['bug_tag']} | {result['provider']}/{result['model']} | {result['status']}{api_suffix}"
